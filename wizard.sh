...
CFVAL=`crudini --get $ROOT/config.ini system swap_enable`
...
if [[ "${swap_enable,,}" =~ ^(yes|y)$ ]] ; then
    CFVAL=`crudini --get $ROOT/config.ini system swap_size`
...
# Nginx + Amplify
#-----------------------------------------------------------------------------------------
CFVAL=`crudini --get $ROOT/config.ini nginx amplify`
...
if [[ "${amplify_install,,}" =~ ^(yes|y)$ ]] ; then
    CFVAL=`crudini --get $ROOT/config.ini nginx api_key`
...
if [[ "${mysql_install,,}" =~ ^(yes|y)$ ]] ; then
    ...
    CFVAL=`crudini --get $ROOT/config.ini mysql bind_address`
    read -ep "Database Bind Address                          : " -i "$CFVAL" bind_address
    SetConfig mysql bind_address $bind_address

    CFVAL=`crudini --get $ROOT/config.ini mysql root_pass`
    ...
    CFVAL=`crudini --get $ROOT/config.ini mysql engine`
...
# PostgreSQL server
#-----------------------------------------------------------------------------------------
CFVAL=`crudini --get $ROOT/config.ini postgres install`
...
if [[ "${pgsql_install,,}" =~ ^(yes|y)$ ]] ; then
    ...
    CFVAL=`crudini --get $ROOT/config.ini postgres bind_address`
    read -ep "Database Bind Address                          : " -i "$CFVAL" bind_address
    SetConfig postgres bind_address $bind_address

    CFVAL=`crudini --get $ROOT/config.ini postgres root_pass`
    read -ep "Set database root password                     : "  -i "$CFVAL" root_pass
    if [[ "$root_pass" == "auto" ]] ; then
...
# Redis server
#-----------------------------------------------------------------------------------------
CFVAL=`crudini --get $ROOT/config.ini redis install`
