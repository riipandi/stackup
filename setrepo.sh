...
if [ "$country" == "ID" ] ; then
  ...
  echo "deb http://mariadb.biz.net.id/repo/10.3/debian `lsb_release -cs` main" > /etc/apt/sources.list.d/lempstack.list
  ...
  echo "deb http://download.nus.edu.sg/mirror/mariadb/repo/10.3/debian `lsb_release -cs` main" > /etc/apt/sources.list.d/lempstack.list
else
